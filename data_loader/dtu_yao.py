...
from data_loader.data_io import read_pfm
import random
...
class MVSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            input_depth_filename = os.path.join(self.datapath, 'inputs/{}/depth_est/{:0>3}_{}.pfm'.format(scan, vid, light_idx))
            inp_depth = np.array(read_pfm(input_depth_filename)[0], dtype=np.float32)
            input_depths.append(inp_depth)
            input_conf_filename = os.path.join(self.datapath, 'inputs/{}/confidence/{:0>3}_{}.pfm'.format(scan, vid, light_idx))
            inp_conf = np.array(read_pfm(input_conf_filename)[0], dtype=np.float32)
            input_confs.append(inp_conf)
        ...
        input_depths = np.expand_dims(np.stack(input_depths), axis=1)
        input_confs = np.expand_dims(np.stack(input_confs), axis=1)

        return {"proj_matrices": proj_matrices_ms,
                "depth": depth_ms,
